...
class UserTest < ActiveSupport::TestCase
  ...
  test 'name not over limit' do
    @user.name = 'a' * 65
    assert_not @user.valid?
  end

  test 'email not over length' do
  ...
  test 'email validation should reject invalid formats' do
    invalid_addresses = %w[user@example,com user_at_foo.org user.name@example.
                           foo@bar_baz.com foo@bar+baz.com extra@dot..io]
  ...
  test 'email addresses should be saved as lower case' do
    mixed_case_email = "BoGuS@EmaIl.CoM"
  ...
  test 'associated microposts should be destroyed' do
    @user.save
    @user.microposts.create!(content: "whatevs, conts")
